...
const login = asyncErrorWrapper(async (req,res,next) => {
    ...
    if(!validateUserInput(email,password)) {
        return next(new CustomError("Your email or password is not valid.",400));
    }

    const user = await User.findOne({email}).select("+password");

    if(!comparePassword(password,user.password)) {
        return next(new CustomError("The password you entered is incorrect.",400));
    }
...
const forgotPassword = asyncErrorWrapper(async (req,res,next) => {
    ...
    const resetPasswordUrl = `http://localhost:5000/api/auth/resetpassword?resetPassword=${resetPasswordToken}`;
...
const editDetails = asyncErrorWrapper(async(req,res,next) => {
    ...
    if(editInfo.hasOwnProperty === "role"){
        return next(new CustomError("You are not authorized to this route.",403));
    }
